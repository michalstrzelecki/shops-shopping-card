<span
    ...
    @add-to-cart.window="$wire.addProduct($event.detail.productId)"
    @click="openDropdown"
>
    ...
    <div x-show="isDropdownOpen && cart.length" @click="isDropdownOpen = false" class="fixed inset-0 h-full w-full z-10"></div>

    <div
        @click.away="closeDropdown"
        x-show="isDropdownOpen && cart.length"
        ...
        <template x-for="(item, index) in cart" :key="item.id">
            <div class="flex flex-row justify-end align-baseline px-4 py-2 text-sm capitalize text-gray-700">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                     class="w-6 h-6 ml-2"
                     @click="$wire.removeProduct(index)"
                >
